...
for x in $(find ./counties -name "*.geojson" -type f | sort); do
	...
	psql -d batch_network -U snow -a -f helper_05_connected_components.sql

	# KNN matching for all nodes in pgrouting table
	psql -d batch_network -U snow -a -f helper_05_knn_match.sql
	...
	psql -d batch_network -U snow -a -f helper_05_cost_matrix.sql.tmp
	rm helper_05_cost_matrix.sql.tmp
